...
$try^{prs}\:k\:err\:pm = lift^{st}(lift^{st}(try^{res} k (\lambda e_1 . try^{res} return^{id} (\lambda e_2. fail^{prs}(e_1 + e_2)))err))pm : (\alpha \rightarrow  Mcons^{prs}_\beta)\rightarrow
...
\BinaryInfC{$lift(try^{res}k\:(\lambda e_1 . try^{res} return^{id}\:(\lambda e_2. fail^{prs}(e_1 + e_2))\:err)\:pm) :
         Mcons^{res}_\beta$ }
\BinaryInfC{$lift(lift(try^{res}k\:(\lambda e_1 . try^{res} return^{id}\:(\lambda e_2. fail^{prs}(e_1 + e_2))\:err)\:pm)) :
         Mcons^{res}_\beta$ }
\end{prooftree}
